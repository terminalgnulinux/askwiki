...
# Script: askwiki.sh
# Descrição: Regex de busca, que procura tópicos no site 'wikipedia'
#-------------------------------------------------------------------#
...
while read line
do
	if [ "$(echo "$line" | egrep -oi "^Procure por $*")" ]; then
		echo -e "\nInfelizmente não encontrei nada a respeito. Desculpe :("
		exit 0
	elif [[ "$(echo "$line" | egrep -o "^Esta é uma página de desambiguação.*")" || $ref -eq 1 ]]; then
		if [ $ref -eq 0 ]; then
			echo -e "\nEncontrei algumas referências sobre o assunto que está procurando."; fi
		echo "$line" | egrep -i "^$**"; ref=1
	else
		[ $info -eq 0 ] && { echo -e "\nEu tenho algumas informações para você, espero que lhe ajude. :)"; info=1; }
		echo "$line"
	fi
...
done < <(curl --use-ascii https://pt.wikipedia.org/wiki/$match | \
		sed -n 's/[^>]*\([^<]*<\)[^>]*/\1/pg' | \
		sed 's/\(.*\)[[].*$/\1/g;s/\(<\|>\)//g;s/\(^(.*$\|^.*&.*;\)//g' | \
		sed '1,/\(Este artigo\|pesquisa\|Esta página ou secção\)/d;/Índice/,$d;/./,/^$/!d')
